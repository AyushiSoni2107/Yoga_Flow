...
export function Blogs() {
  ...
  const fetchPosts = async () => {
    try {
      const data = await apiRequest<BlogPost[]>('/blogs');
      setPosts(data || []);
    } catch (error) {
          ...
          {posts.map((post) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-gray-600 mb-2">By {post.author_name || 'Yoga Flow Team'}</p>
                <h2 className="text-2xl font-bold text-gray-900 mb-3">{post.title}</h2>
                <p className="text-gray-600 mb-4 line-clamp-3">{post.excerpt}</p>
                <Link
                  href={user ? `/blogs/details?slug=${encodeURIComponent(post.slug)}` : '/login'}
                  className="flex items-center text-violet-600 hover:text-violet-700 font-semibold transition-colors"
                >
                  Read more
